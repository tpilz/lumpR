# Template R Script for pre-processing of WASA model using LUMP package and GRASS GIS
# (see https://github.com/tpilz/LUMP/wiki for extended information on usage)
...
# (see https://github.com/tpilz/LUMP/wiki for extended information)
#
# 3.
# adjust this template script and process line by line
# get help for commands by typing e.g.
# ?lump_grass_prep
...
# switch to specified working directory
  setwd("/home/tobias/Promotion/Modellierung/WASA/LUMP/scripts/") #use "/" instead of "\" in Windows

# start GRASS session
  initGRASS(gisBase="/opt/grass",                                     # path to GRASS installation (use / instead of \ under windows, e.g. "d:/programme/GRASS6.4.3" )
           ...
           gisDbase="/home/tobias/Promotion/Modellierung/grassdata",  # path to grass data directory containing the location specified above and all corresp. data
           override=TRUE)
...
# (optional) clean up GRASS location if necessary (remove outputs from previous call of lump_grass_prep())
  #execGRASS("g.mremove", rast="*fill,stream,stream_horton,eha,flow_dir,flow_accum,elev_riv,dist_riv,MASK*,svc*", flags=c("f"))




### (optional) SUBBASIN DELINEATION ###
