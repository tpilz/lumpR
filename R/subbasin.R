...
calc_subbas <- function(
  ...
  ver_no = gsub(GRASSver, pattern="GRASS *([^\\.]).*", repl="\\1") #extract main gras version
 ...
 tryCatch({
  ...
  if(is.null(river)) {
    ...
    res <- c(res$nsres, res$ewres)
    
    # create shifted version of drainage points (shifted by 1/4 of resolution to East, 1/3 of resolution to North)
    drain_points_shifted <- drain_points_centered
    drain_points_shifted@coords <- drain_points_shifted@coords + c(1/3, 1/4) * res
  ...
  #     if specified subbas too small, remove upstream neighbours instead and merge with specified subbas
        ...
        for(i in 1:length(cmd_out))
        {
          ...
          cmd_out2[[i]] =  na.omit(as.numeric(cmd_out[[i]][-1])) #save the IDs of all subbasins overlapping with the current one
        ...
        if(rm_spurious>0) {
          ...
          sub_sizes_int <- matrix(as.numeric(unlist(strsplit(cmd_out, " "))), ncol=2, byrow=T)
          #sub_sizes_int <- sub_sizes_int[-which(sub_sizes_int[,1] == 0),]
          sub_rm_int <- sub_sizes_int[which(sub_sizes_int[,2] < rm_spurious*thresh_sub),1] #internal subbas IDs below removal threshold
          ...
          # get external subbas IDs to be removed
            sub_rm_ext <- as.numeric(basins_points[which(as.numeric(basins_points[,1]) %in% sub_rm_int),2]) 
            #browser()
            ...
            if (length(manually_specified_ext)>0) #if manually specified external subbas ID detected as "too small", remove their upstream neighbours instead
            {
              for (cur_sub in manually_specified_ext) #cur_sub = external ID
              {
                ...
                tt =unlist(lapply(cmd_out2, FUN= function(x,y){length(c (setdiff(x, y), setdiff(y,x)) )}, y=curr_line)) #number of differences in configuration; 0 = currently active subbas; 1 = nearest upstream or downstream neighbour
                
                #FUN(x=cmd_out2[[13]], y= curr_line)
                
                upstream_neighbours_int =  which(tt == 1 ) #internal subbas ID of upstream neighbours
