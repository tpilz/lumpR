...
#' @param river (optional) River vector map in GRASS location if available - use only if this vector map is strictly consistent
...
#' @param drainage_dir (optional) Existing raster map of drainage direction in GRASS location(saves computation time). Must correspond to map generated by the GRASS function \code{r.watershed}.
#'      If \code{drainage_dir} or \code{flowaccum} is set to \code{NULL} (default value), both will be calculated with \emph{r.watershed}.
#' @param disk_swap (optional) Only needed if memory requirements exceed available RAM (i.e. for very large DEMs): If set to \code{TRUE}, \code{r.watershed} uses the "-m"-flag (slower, but runs with larger DEMs)
...
#' @param points_processed Output: Prefix of intermediate point vector files exported to GRASS location, if \code{keeptemp = TRUE}, which can be used for checking in terms of unexpected behaviour.
...
#'      contains only one point, this will be used as catchment outlet.
...
#'      Default: \code{NULL}.
#' @param thresh_sub Integer defining threshold for (additional) subbasin calculation. Parameter for
...
#'      given drainage points are used for subbasin delineation, i.e. no further subbasins created.
...
#'      Spurious subbasins are 'interior' watersheds sometimes created by GRASS function
#'      \emph{r.watershed} around stream segments below multiple tributaries. These
#'      very small subbasins produce induce unnecessary computational burden when used within a
#'      hydrological model. If removed, these areas will be merged to the next upstream
...
#'      
#' @details
#'  The function delineates subbasins. It can use user-specified outlet points via parameter \code{drain_points}, and/or the area-threshold \code{param thresh_sub}.
#'  The user-specified outlet points are snapped to the nearest river segment (to fix inaccuracies) and will result in mandatory subbasins.
#'  The area-threshold serves for delineation of further subbasins (if necessary), and potential merging of too-small subbasins to their upstream neighbours.
...
#'      to the wrong stream segment, adjust outlet point locations manually in GRASS and re-run
...
calc_subbas <- function(
  ...
  if(is.null(outlet)) {
    if(nrow(drain_points@coords) > 1)
      stop("You have to specify 'outlet' if there a more than one points in 'drain_points'!")
    outlet <- 1
  }
  ...
  if(is.null(river)) {
    ...
    if(is.numeric(thresh_sub)) {
      ...
      if ((!is.null(attr(cmd_out, "status")) && attr(cmd_out, "status")!=0))
        stop(paste0("Could not get zonal stats of 'accum_t' (flow accumulation) and 'basin_calc_t' (automatically calculated subbasins). Check both rasters."))
    ...
    no_catch <- nrow(drainp_coords) #current number of subbasins
    ...
    if(no_catch > 1) {
      ...
      while (TRUE) {
        ...
        if(iteration_nr > 10)
        {
          if(!silent) message("% Could not resolve all spurious subbasins, iteration limit exceeded. Check results manually.")
        ...
        if(rm_spurious>0) {
          if(!silent) message(paste("% Iteratively remove spurious subbasins ...(Iteration:",iteration_nr,")"))
          ...
          { #no changes
            if(!silent) message("% Could not resolve some subbasins below threshold (", paste0(sub_rm_int, collapse=", "),"). Refine manually, if required.")
            break # exit while loop
          } else
    ...
    if(!silent) message("% -> Note: Subbasin IDs of the calculated map 'subbas' might not be consecutively numbered! Fix this manually, if required.")
    if(!silent) message("%")
