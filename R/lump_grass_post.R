...
#'      be created with \code{\link[LUMP]{lump_grass_prep}}.
#' @param soil_depth Name of soil depth [cm] raster map in GRASS location.
...
lump_grass_post <- function(
  ...
  tryCatch({
    ...
    if (file.exists(paste(dir_out,sub_ofile,sep="/")))
      stop(paste("Output file ", sub_ofile, " already exists in ", dir_out, "!", sep=""))
    
    if (file.exists(paste(dir_out,lu_ofile,sep="/")))
      stop(paste("Output file ", lu_ofile, " already exists in ", dir_out, "!", sep=""))
    ...
    execGRASS("r.reclass", input=eha, output=lu, rules=recl_lu)
  ...
  ### SUBBASIN PARAMETERS ###
      ...
      if(!is.finite(flowtime_med) || !is.finite(retention))
        stop(paste0("Could not calculate finite subbasin parameters for subbasin ", SUB))
  ...
  ### Landscape Unit statistics ###
    ...
    execGRASS("g.remove", rast="MASK_t", flags=c("f"))
  ...
  ### LANDSCAPE UNIT PARAMETERS ###
    ...
    lu_par <- cbind(as.numeric(cmd_out[,"zone"]), lu_depth)
