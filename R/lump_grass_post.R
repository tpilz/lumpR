  ...
  tryCatch({
    ...
    if (is.null(recl_lu)) #use existing LU-map
    {
      ...
      {        
        ...
        x=execGRASS("r.mapcalc", expression=paste0("grow_eval_t = if(isnull(", lu, "), 1, null())"), flags = "overwrite", intern=TRUE)
        
        na_eval <- execGRASS("r.stats", input=paste0("grow_eval_t,", mask), flags=c("n","p"), intern=TRUE, ignore.stderr = T)
        
        if (length(na_eval)==0 | !fill_holes) {  
          x=execGRASS("g.remove", type="raster", name="grow_eval_t", intern=TRUE)
        ...
        if(!silent) message(paste0("% -> ", tail(unlist(na_res),1), " NAs in ", lu))
        
        # otherwise grow LU map to fill gaps 
        if(!silent) message(paste0("% -> Filling empty patches in ",lu,", iterating..."))
        x=execGRASS("r.grow",   input=lu, output=lu, radius=GROWRAD, flags="overwrite", intern=TRUE)
        
        # r.grow converts type CELL to type DCELL; convert back to CELL
        cmd_out <- execGRASS("r.mapcalc", expression=paste0(lu, "=int(", lu, ")"), flags = "overwrite", intern = T)
